...
const char default_url[] = "https://dns.google/dns-query";

struct proxy_config 
{
  bool use_proxy;
  int proxy_type;
  char proxy_host_port[BUFSIZ];
};
...
static int initprobe(int dnstype, char *host, const char *url, CURLM *multi,
                     ...
                     struct curl_slist *resolve, struct proxy_config *config)
{
  CURL *curl;
  ...
  if(curl) {
    ...
    if (config->use_proxy) {
      curl_easy_setopt(curl, CURLOPT_PROXY, config->proxy_host_port);
...
static void 
parse_proxy(struct proxy_config *config, char *url)
{
  int rc;
  CURLU *proxy = NULL;
  char *proxy_host = NULL, *proxy_type = NULL, *proxy_port = NULL;
  proxy = curl_url();
  
  if ((rc = curl_url_set(proxy, CURLUPART_URL, url, CURLU_NON_SUPPORT_SCHEME))) {
    goto clean;
  }
  if ((rc = curl_url_get(proxy, CURLUPART_HOST, &proxy_host, 0))) {
    goto clean;
  }
  if ((rc = curl_url_get(proxy, CURLUPART_SCHEME, &proxy_type, 0))) {
    goto clean;
  }
  if ((rc = curl_url_get(proxy, CURLUPART_PORT, &proxy_port, 0))) {
    goto clean;
  }
  
  if (!strcmp(proxy_type, "http")) config->proxy_type = CURLPROXY_HTTP;
  else if (!strcmp(proxy_type, "https")) config->proxy_type = CURLPROXY_HTTPS;
  else if (!strcmp(proxy_type, "socks")) config->proxy_type = CURLPROXY_SOCKS4;
  else if (!strcmp(proxy_type, "socks4")) config->proxy_type = CURLPROXY_SOCKS4;
  else if (!strcmp(proxy_type, "socks4a")) config->proxy_type = CURLPROXY_SOCKS4A;
  else if (!strcmp(proxy_type, "socks5")) config->proxy_type = CURLPROXY_SOCKS5;

  bzero(config->proxy_host_port, BUFSIZ * sizeof(char));
  sprintf(config->proxy_host_port, "%s:%s", proxy_host, proxy_port);
  
  clean:
    if(proxy_host) free(proxy_host);
    if(proxy_type) free(proxy_type);
    if(proxy_port) free(proxy_port);
    curl_url_cleanup(proxy);

    if (rc) {
      help(curl_easy_strerror(rc));
    }
}
...
static void help(const char *msg)
{
  ...
  fprintf(stderr, "Usage: doh [options] <host> [URL]\n"
        ...
        "  -x  use proxy [protocol://]host[:port] It supports both SOCKS(4)5 and HTTP proxies\n"
        "  -V  show version\n"
        "(default URL is %s)\n",
        default_url);
  exit(1);
...
int main(int argc, char **argv)
{
  ...
  bzero(&config, sizeof(struct proxy_config));
